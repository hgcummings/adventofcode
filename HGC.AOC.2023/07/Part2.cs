...
public class Part2 : ISolution
{
    ...
    public const string CardVals = "J23456789TQKA";

    public static HandType GetHandType(String hand)
    {
        var groups = hand.GroupBy(c => c)
            .OrderByDescending(g => g.Count())
            .ThenByDescending(g => CardVals.IndexOf(g.Key))
            .ToList();
        
        if (hand.Contains('J'))
        {
            var proxyHand = (hand == "JJJJJ") ? "AAAAA" : hand.Replace('J', groups.First(g => g.Key != 'J').Key);
            groups = proxyHand.GroupBy(c => c).OrderByDescending(g => g.Count()).ToList();
        }
