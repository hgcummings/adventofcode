...
const initialState = [bossInitialHp,50,500,0,0,0];
...
var best = Infinity;
while(!queue.isEmpty()) {
    let state = queue.dequeue().state;
    
    if (won(state)) {
        continue;
    }
    
    var [startOfPlayerTurn] = applyEffects(state);

    for (const option of options) {
        if (option.applicable(state)) {
            const endOfPlayerTurn = option.apply(startOfPlayerTurn);
            let manaSpent = startOfPlayerTurn[2] - endOfPlayerTurn[2];
            let newG = g.get(state) + manaSpent;
            let newChain = chain.get(state) + "," + manaSpent;

            if ((endOfPlayerTurn[0] <= 0)) {
                if (newG < best) {
                    best = newG;
                    console.log(best);
